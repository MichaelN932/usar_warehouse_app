    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">Warehouse performance and inventory insights</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          color={metrics.lowStockCount > 5 ? 'fire' : 'default'}
        ...
        <Card title="Request Status Breakdown">
          <div className="space-y-4">
            {[
              ...
              { label: 'Fulfilled', count: requests.filter((r) => r.status === 'Fulfilled').length, color: 'bg-gray-500' },
              ...
              topRequestedItems.map((item, index) => (
                <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div className="flex items-center">
                    <span className="w-6 h-6 rounded-full bg-fire-100 text-fire-700 flex items-center justify-center text-sm font-medium mr-3">
                      {index + 1}
                    </span>
                    <span className="font-medium text-gray-900">{item.name}</span>
                  </div>
                  <span className="badge bg-gray-100 text-gray-800">{item.count} requested</span>
                </div>
              ))
            )}
          </div>
          ...
          {lowStockItems.length === 0 ? (
            ...
            <div className="space-y-2">
              {lowStockItems.slice(0, 5).map((item) => (
                ...
                >
                  <div>
                    <p className="font-medium text-gray-900">{item.itemTypeName}</p>
              ...
              return (
                <div key={categoryName} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">{categoryItems.length} SKUs</p>
                  </div>
                  <span className="text-lg font-semibold text-gray-900">{totalQty}</span>
                </div>
              );
            })}
          </div>
      ...
      <Card title="System Overview">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          <div className="text-center p-4 bg-gray-50 rounded-lg">
            <div className="text-2xl font-bold text-gray-900">{catalogItems.length}</div>
          ...
          <div className="text-center p-4 bg-gray-50 rounded-lg">
            <div className="text-2xl font-bold text-gray-900">{issuedItems.length}</div>
            <div className="text-sm text-gray-500">Items Issued</div>
          </div>
          <div className="text-center p-4 bg-gray-50 rounded-lg">
          ...
          <div className="text-center p-4 bg-gray-50 rounded-lg">
            ...
            <div className="text-sm text-gray-500">All-time Requests</div>
          </div>
        </div>
      </Card>
    </div>
