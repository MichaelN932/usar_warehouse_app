...
export function Profile() {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div>
        <h1 className="text-heading-lg text-primary-900">Profile</h1>
        <p className="text-primary-600">Manage your account settings and sizes</p>
      </div>
      ...
      <Card title="Account Information">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-primary-500">Name</label>
            <p className="text-primary-900 font-medium">
              {user?.firstName} {user?.lastName}
            </p>
          </div>
          <div>
            <label className="block text-sm font-medium text-primary-500">Email</label>
            <p className="text-primary-900">{user?.email}</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-primary-500">Role</label>
          ...
          <div>
            <label className="block text-sm font-medium text-primary-500">Member Since</label>
            <p className="text-primary-900">
          ...
          {sizeFields.map(({ key, label, options }) => (
            <div key={key}>
              ...
              {isEditing ? (
                ...
                <p className="text-primary-900 py-2">
                  {sizes[key as keyof UserSizes] || (
                    <span className="text-primary-400">Not set</span>
                  )}
                </p>
              )}
            </div>
        ...
        <div className="flex flex-wrap gap-3">
          {user?.sizes?.shirt && (
            <div className="bg-primary-50 rounded-lg px-4 py-2">
              <span className="text-xs text-action-primary block">Shirt</span>
              <span className="font-semibold text-primary-800">{user.sizes.shirt}</span>
            </div>
          )}
      ...
      <Card title="Notification Preferences">
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="font-medium text-primary-900">Request Updates</p>
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="font-medium text-primary-900">Ready for Pickup</p>
              <p className="text-sm text-primary-500">
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="font-medium text-primary-900">System Updates</p>
              <p className="text-sm text-primary-500">
