...
export interface User {
  ...
  employeeId?: string;      // Badge/employee number
  department?: string;      // e.g., 'USAR', 'Fire Station 88', 'Admin'
  position?: string;        // e.g., 'Task Force Leader', 'Rescue Specialist', 'Logistics'
  phone?: string;
  hireDate?: string;
  // PPE tracking (populated from API)
...
// Inbound Email (for email integration)
...
export interface ProcurementMetrics {
  ...
  budgetsByGrant: BudgetByGrant[];
}

export interface BudgetByGrant {
  grantId: string;
  grantName: string;
  grantCode: string;
  totalBudget: number;
